...
and fix them; assign them to a milestone (like <code>0.12.0-6</code>).
...
- to remove the rest of the dates, change from:
  ...
  *
  ```
- add a new entry like _**\* v0.12.0-6 prepared**_
- **commit** with the message _**prepare v0.12.0-6**_
...
tagged like **v0.12.0-6** (mind the dash in the middle!) and
named like **xPack OpenOCD v0.12.0-6** (mind the dash),
...
- add entries in the **Bug fixes**; use the following syntax
  ```
  - [[#N](https://github.com/xpack-dev-tools/openocd-xpack/issues/N)]:
  ```
...
- copy/paste the folders hierarchy from the install guide
- **commit** with the message _**website: blog post release 0.12.0-6 published**_
...
- **commit** all changes with the message
  _**package.json: update urls for 0.12.0-6.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v0.12.0-6.1 published on npmjs.com**_
- **commit** with the message _**CHANGELOG: publish npm v0.12.0-6.1**_
...
`npm dist-tag add @xpack-dev-tools/openocd@0.12.0-6.1 latest
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/openocd@0.12.0-6.1
`} </CodeBlock>
...
- paste the release name like _**xPack OpenOCD v0.12.0-6 released**_
