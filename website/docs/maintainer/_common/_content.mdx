...
Run the `generate-top-commons` npm script from the top `package.json`.

### Update the website commons

Run the `generate-website-commons` npm script from `website`.
...
and fix them; assign them to a milestone (like <code>0.12.0-5</code>).
...
- to remove the rest of the dates, change from:
  ...
  *
  ```
- add a new entry like _**\* v0.12.0-5 prepared**_
- commit with the message _**prepare v0.12.0-5**_
...
(like **0.12.0**) with the following expression:
...
```sh
npm run generate-top-commons -C ~/Work/xpack-dev-tools/openocd-xpack.git
```
...
```sh
xpm run clear-all-projects-builds -C ~/Work/xpack-dev-tools/openocd-xpack.git/build-assets
```
...
tagged like **v0.12.0-5** (mind the dash in the middle!) and
named like **xPack OpenOCD v0.12.0-5** (mind the dash),
...
- commit with the message _**website: blog post release 0.12.0-5 published**_
...
- commit all changes with the message
  _**package.json: update urls for 0.12.0-5.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v0.12.0-5.1 published on npmjs.com**_
- commit with the message _**CHANGELOG: publish npm v0.12.0-5.1**_
...
<CodeBlock language="sh"> {
`npm dist-tag add @xpack-dev-tools/openocd@0.12.0-5.1 latest
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/openocd@0.12.0-5.1
`} </CodeBlock>
...
- paste the release name like _**xPack OpenOCD v0.12.0-5 released**_
...
[![NPM Version](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fopenocd?color=green)](https://www.npmjs.com/package/@xpack-dev-tools/openocd/)
[![NPM Downloads](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fopenocd)](https://www.npmjs.com/package/@xpack-dev-tools/openocd/)
  - all releases [![NPM Downloads](https://img.shields.io/npm/dt/%40xpack-dev-tools%2Fopenocd)](https://www.npmjs.com/package/@xpack-dev-tools/openocd/)
